...
class ElevatedButtonAddProduct extends StatefulWidget {
  ...
  const ElevatedButtonAddProduct(this.controller, {Key? key}) : super(key: key);
  ...
  @override
  State<ElevatedButtonAddProduct> createState() => _ElevatedButtonAddProductState();
}

class _ElevatedButtonAddProductState extends State<ElevatedButtonAddProduct> {
  @override
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: () async {
          ...
          if(isValidForm){
            var response = await widget.controller.validateEmptyFields(context);
            if(response) {
              showDialog(
                ...
                builder: ((context) => DefaultAlertDialogOneButton(
                  ...
                  confirmText: 'Ok',
                  onConfirm: () {
                    // Substituído pushAndRemoveUntil por pop para voltar à tela anterior
                    Navigator.of(context).pop(); // Fecha o diálogo
                    Navigator.of(context).pop(); // Volta para a tela de listagem
                    
                    // Se necessário, atualize a tela de listagem
                    // Se ListProductsScreen.controller é acessível
                    if (Get.isRegistered<ListProductsController>()) {
                      Get.find<ListProductsController>().fetchProducts();
                    }
